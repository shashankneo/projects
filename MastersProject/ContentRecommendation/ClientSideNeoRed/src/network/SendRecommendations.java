...
public class SendRecommendations {
	private Client client = null;
    private  boolean sent = true;
    private  boolean done = false;
	private Logger logger = LoggerFactory.getLogger(SendRecommendations.class);
	
	public boolean sendRecommendationsLinks(){
	 ...
	 boolean sendDataToServer(final String date, final String jsonData){
		        ...
		        client.addListener(new Listener() {
		            public void connected(Connection connection) {
		                ...
		                request.command = Constants.cmd_SendlaptopRecommendation;
		            ...
		            public void received(Connection connection, Object object) {
		            	logger.info("Send recommendation response received from server");
		                if (object instanceof SomeResponse) {
		                    SomeResponse response = (SomeResponse) object;
		                    if(response.command.equals(Constants.cmd_SendlaptopRecommendationDone)){   
		                        		sent = true;
		                                Util.setPreferences(Constants.pref_dateShareRecommendations, date);
		                                logger.info("Send recommendation response received from server for date ="+response.date);
		                    }       
		                    else{
		            ...
		            public void disconnected(Connection connection) {
		                ...
		                sent = false;
		                logger.error("Send recommendation response not received for date ="+ date);
		            }
		        });
		        ...
		        try {
		        	...
		        	logger.error("Exception during recommendation response data not received for date ="+ e.toString());
		        	done = true;
		        }
		        while(!done);
	 ...
	 public  String findNextFolderToShare() {
	        ...
	        String lastFolder = Util.getPreferences(Constants.pref_dateShareRecommendations);
